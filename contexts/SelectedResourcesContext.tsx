...
export function SelectedResourcesProvider({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const storedAuthorResourcesIds =
        localStorage.getItem('authorResourcesIds');
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        try {
          ...
          console.error(
            'Error saving author resource IDs to localStorage:',
            error
          );
        }
      }
  ...
  useEffect(() => {
    ...
    if (
      ...
      (current.length > 0 &&
        previous.length > 0 &&
        current[0].id !== previous[0].id)
    ) {
      // Log removal when going from 1 to 0 (direct removal)
      if (previous.length === 1 && current.length === 0) {
        const removedResourceName =
          (previous[0] as any).title || (previous[0] as any).name || 'Resource';
      ...
      if (current.length > 0) {
        const selectedResourceName =
          (current[0] as any).title || (current[0] as any).name || 'Resource';
  ...
  const addResource = useCallback(
    async (
      ...
      if (
        ...
        lastAction.resourceId === resource.id &&
      ...
      setSelectedResources((prev) => {
        ...
        if (typeof window !== 'undefined') {
          try {
            ...
            console.error(
              'Error saving selected resources to localStorage:',
              error
            );
          }
        }
  ...
  const removeResource = (resourceId: string | number) => {
    ...
    if (
      ...
      lastAction.resourceId === resourceId &&
      now - lastAction.timestamp < 100
    ) {
      return;
    }
    ...
    setSelectedResources((prev) => {
      ...
      if (typeof window !== 'undefined') {
        try {
          ...
          console.error(
            'Error updating selected resources in localStorage:',
            error
          );
        }
      }
  ...
  const resetSelectedResources = () => {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        console.error(
          'Error clearing selected resources from localStorage:',
          error
        );
      }
    }
