...
export async function getAuthors(params?: {
  ...
  if (params?.query) {
    ...
    filteredAuthors = filteredAuthors.filter(
      (author) =>
        author.name.toLowerCase().includes(lowercaseQuery) || author.bio.toLowerCase().includes(lowercaseQuery),
    )
  }
  if (params?.limit) {
...
export async function getAuthor(id: string): Promise<Author & { resources: Resource[] }> {
