...
export function PeopleCardListMobile({ people, isLoading }: PeopleCardListProps) {
  ...
  useEffect(() => {
    if (!scrollRef.current || isLoading || !selectedResources.length || !people.length) return;
    
    const selectedAuthor = selectedResources[0];
    const selectedIndex = people.findIndex(person => person.id === selectedAuthor.id);
  ...
  return (
    <div className="md:hidden space-y-4">
      ...
      <div className="relative">
        ...
        <div 
          ref={scrollRef}
          className="flex gap-6 overflow-x-auto scrollbar-hide pb-2 px-4"
          style={{ 
            scrollbarWidth: 'none', 
