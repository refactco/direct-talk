...
export async function GET(request: Request) {
  ...
  if (topic) {
    filteredResources = filteredResources.filter((resource) => resource.topics.includes(topic))
  }

  if (authorId) {
    filteredResources = filteredResources.filter((resource) => resource.authorId === authorId)
  }
  ...
  if (sort === "popular") {
    ...
    filteredResources.sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime())
  }
